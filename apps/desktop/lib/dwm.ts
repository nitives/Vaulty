...
import type { BackgroundMaterial } from "./settings";
...
let dwmEnableBlurBehindWindow:
  | ((hwnd: number | bigint, pBlurBehind: Buffer) => number)
  | null = null;
...
function ensureFFI(): boolean {
  ...
  try {
    ...
    dwmSetWindowAttribute = dwmapi.func(
      "int32 __stdcall DwmSetWindowAttribute(intptr hwnd, uint32 dwAttribute, void *pvAttribute, uint32 cbAttribute)",
    );
    ...
    dwmEnableBlurBehindWindow = dwmapi.func(
      "int32 __stdcall DwmEnableBlurBehindWindow(intptr hwnd, void *pBlurBehind)",
    );
...
function readHwnd(buf: Buffer): number | bigint {
...
function setDwmUint32(
  ...
  if (hr !== 0) {
    console.warn(
      `[DWM] DwmSetWindowAttribute(${attr}) returned HRESULT 0x${(hr >>> 0).toString(16)}`,
    );
  }
}

function enableBlurBehind(hwnd: number | bigint): void {
  ...
  if (hr !== 0) {
    console.warn(
      `[DWM] DwmEnableBlurBehindWindow returned HRESULT 0x${(hr >>> 0).toString(16)}`,
    );
  }
}

function materialToBackdropType(material: BackgroundMaterial): number {
  switch (material) {
    case "acrylic":
 ...
 */
export function restoreDwmAttributes(
  win: BrowserWindow,
  ...
  setDwmUint32(
    ...
    materialToBackdropType(material),
  );
  ...
  console.log(
    `[DWM] Restored blur-behind + corner=ROUND + backdrop=${material}`,
  );
}
 ...
 */
export function clearDwmBackdrop(win: BrowserWindow): void {
  if (process.platform !== "win32") return;
  if (!ensureFFI()) return;
